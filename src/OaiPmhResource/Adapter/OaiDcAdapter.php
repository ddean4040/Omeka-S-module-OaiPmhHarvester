 ...
 * @subpackage Models
 */
class OaiDcAdapter extends AbstractAdapter
{
    ...
    protected $_elements = array('contributor', 'coverage', 'creator',
    ...
    /**
     * Return elements with Dublin Core prefix attached
     */
    public function getElements() {
        $elements = array();
        foreach( $this->_elements as $element ) {
            $elements[] = self::DUBLIN_CORE_PREFIX . ':' . $element;
        }
        return $elements;
    }
    
    /**
     * Retrieve Dublin Core elements from an XML record blob
     */
        ...
        foreach ($this->_elements as $element) {
            if (isset($dcMetadata->$element)) {
                foreach ($dcMetadata->$element as $rawText) {
                    ...
                    $elementTexts[ self::DUBLIN_CORE_PREFIX . ':' . $element ][]
    ...
    // None of the functions below are used by Omeka S
