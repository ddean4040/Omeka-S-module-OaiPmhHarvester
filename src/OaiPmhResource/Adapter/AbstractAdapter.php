...
abstract class AbstractAdapter
{
     ...
     * @return void
     */
    public function __construct( string $formatURL )
    {   
        // Set an error handler method to record run-time warnings (non-fatal 
        // errors).
//        set_error_handler(array($this, 'errorHandler'), E_WARNING);
        
        if( $formatURL != $this::METADATA_SCHEMA ) {
            
            if( ! empty( $this::METADATA_SCHEMA_REGEX ) ) {
                
                if( false === preg_match( sprintf( '|%s|', $this::METADATA_SCHEMA_REGEX ), $formatURL ) ) {
                    throw new \Exception( sprintf( 'Not a regex match: %s vs %s', $formatURL, $this::METADATA_SCHEMA_REGEX ) );
                }
            } else {
                throw new \Exception( sprintf( 'Not a match: %s vs %s', $formatURL, $this::METADATA_SCHEMA ) );
            }
        }
        
        $this->_formatURL = $formatURL;
   
    }
    
    /**
     * Return a list of the elements available in this schema
     */
     ...
     */
    abstract public function extractRecord($record);
